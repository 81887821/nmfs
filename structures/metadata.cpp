...
ssize_t metadata::write(const byte* buffer, size_t size_to_write, off_t offset) {
    ...
    auto data_key = nmfs::structures::utils::data_object_key(uuid, static_cast<uint32_t>(offset / context.maximum_object_size));
...
void metadata::flush() const {
    if (dirty) {
        ...
        memcpy(on_disk_structure.uuid, uuid.data(), uuid.size());
...
void metadata::remove_data_objects(uint32_t index_from, uint32_t index_to) {
    ...
    log::information(log_locations::file_data_operation) << __func__ << " uuid = " << uuid <<"\n";
    auto data_key = nmfs::structures::utils::data_object_key(uuid, index_from);
